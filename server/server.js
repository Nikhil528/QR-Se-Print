...
async function api(req,res,u){
 ...
 if(p==='/api/shop/earnings-breakdown'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false,error:'Unauthorized'});const js=db.jobs.filter(j=>j.shopId===s.id);const paidJobs=js.filter(j=>['paid','counter'].includes(String(j.payment_status||'').toLowerCase()));const now=new Date();const dayKey=d=>new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit'}).format(d);const start=new Date(now.getTime()-6*86400000),startKey=dayKey(start),prevStart=new Date(now.getTime()-13*86400000),prevStartKey=dayKey(prevStart);const thisWeek=paidJobs.filter(j=>dayKey(new Date(j.created_at||0))>=startKey).reduce((a,j)=>a+Number(j.amount||0),0);const lastWeek=paidJobs.filter(j=>dayKey(new Date(j.created_at||0))>=prevStartKey&&dayKey(new Date(j.created_at||0))<startKey).reduce((a,j)=>a+Number(j.amount||0),0);const daily=[];for(let i=6;i>=0;i--){const d=new Date(now.getTime()-i*86400000),k=dayKey(d);daily.push({day:k,amount:paidJobs.filter(j=>dayKey(new Date(j.created_at||0))===k).reduce((a,j)=>a+Number(j.amount||0),0)});}return json(res,200,{success:true,bw:paidJobs.filter(j=>!String(j.colorMode||'').toLowerCase().includes('color')).reduce((a,j)=>a+Number(j.amount||0),0),color:paidJobs.filter(j=>String(j.colorMode||'').toLowerCase().includes('color')).reduce((a,j)=>a+Number(j.amount||0),0),this_week:thisWeek,last_week:lastWeek,daily});}

...
const DEFAULT_SETTINGS = {
  ...
  printers:{models:['Default Printer','Microsoft Print to PDF','HP LaserJet Pro MFP M126 plus series','EPSON L3210 Series']},
  features:{advance:['4x6 Photo','Resume','A3','Duplex','Mini Print']},
  payments:{setupFee:0,currency:'INR',registrationGateway:'razorpay',shopGatewayFallback:'razorpay'},
...
function normalizeShop(s){ if(!s.plan)s.plan='demo'; if(s.plan==='demo'&&!s.demo_expires_at&&s.created_at)s.demo_expires_at=new Date(Date.parse(s.created_at)+Number(CFG().demo.durationDays||1)*86400000).toISOString(); if(typeof s.demo_prints_used!=='number')s.demo_prints_used=0; if(!s.plan_status)s.plan_status='active'; return s;}
...
async function api(req,res,u){
 ...
 if(p==='/api/admin/overview'&&req.method==='GET'){
   ...
   const todayJobs=db.jobs.filter(j=>String(j.created_at||'').slice(0,10)===today&&paid(j));
 ...
 if(p==='/api/admin/shop/update'&&req.method==='POST'){if(!adminAuth(req))return json(res,401,{success:false,error:'Unauthorized'});const x=await bodyJson(req);const s=db.shops.find(q=>String(q.id).toUpperCase()===String(x.shopId||'').toUpperCase());if(!s)return json(res,404,{success:false,error:'Shop not found'});for(const k of ['name','phone','email','plan','plan_status','paused','subscription_expired'])if(k in x)s[k]=x[k];if(x.plan&&PLANS()[x.plan]){s.plan=x.plan;s.plan_status='active';if(x.plan==='demo'){s.demo_expires_at=new Date(Date.now()+Number(CFG().demo.durationDays||1)*86400000).toISOString();s.demo_print_limit=Number(CFG().demo.printLimit);s.demo_prints_used=0;}}save();return json(res,200,{success:true,shop:publicShop(s.id)});}
 ...
 if(p==='/api/shop/advance-active'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false});return json(res,200,{success:true,active:!!PLANS()[s.plan]?.advance});}
 if(p==='/api/shop/advance-module'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false});return json(res,200,{success:true,active:!!PLANS()[s.plan]?.advance});}

...
const RZP_KEY_ID = process.env.PLATFORM_RAZORPAY_KEY_ID || process.env.RAZORPAY_KEY_ID || '';
const RZP_KEY_SECRET = process.env.PLATFORM_RAZORPAY_KEY_SECRET || process.env.RAZORPAY_KEY_SECRET || '';
const PLAN_PRICES = { demo:0, starter:599, pro:899, premium:999 };
function planPrice(plan){ return PLAN_PRICES[String(plan||'').toLowerCase()] || 0; }
async function razorpayOrder(amount, receipt){ if(!RZP_KEY_ID||!RZP_KEY_SECRET) throw new Error('Platform Razorpay is not configured. Add PLATFORM_RAZORPAY_KEY_ID and PLATFORM_RAZORPAY_KEY_SECRET in Render Environment.'); const auth=Buffer.from(RZP_KEY_ID+':'+RZP_KEY_SECRET).toString('base64'); const r=await fetch('https://api.razorpay.com/v1/orders',{method:'POST',headers:{Authorization:'Basic '+auth,'Content-Type':'application/json'},body:JSON.stringify({amount:Math.round(amount*100),currency:'INR',receipt:String(receipt).slice(0,40),payment_capture:1})}); const d=await r.json().catch(()=>({})); if(!r.ok) throw new Error(d.error?.description||'Razorpay order creation failed'); return d; }
function verifyRazorpayPayment(orderId,paymentId,signature){ const expected=crypto.createHmac('sha256',RZP_KEY_SECRET).update(String(orderId)+'|'+String(paymentId)).digest('hex'); const got=String(signature||''); return got.length===expected.length && crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(got)); }
function generateShopId(){ let sid='SHOP'+Math.floor(100000+Math.random()*900000); while(db.shops.some(s=>s.id===sid)) sid='SHOP'+Math.floor(100000+Math.random()*900000); return sid; }
function generatePassword(){ return crypto.randomBytes(4).toString('hex').toUpperCase(); }
...
function publicShop(id0){const s=db.shops.find(x=>x.id.toUpperCase()===String(id0).toUpperCase());if(!s)return null;return {...s,passwordHash:undefined,agent_token:undefined};}
...
async function api(req,res,u){
 ...
 if(p==='/api/payment/online/create'&&req.method==='POST'){const x=await bodyJson(req);const j=db.jobs.find(q=>q.id===x.jobId);if(!j)return json(res,404,{success:false,error:'Job not found'});try{const o=await razorpayOrder(j.amount,j.id);j.razorpay_order_id=o.id;save();return json(res,200,{success:true,keyId:RZP_KEY_ID,orderId:o.id,amount:o.amount,currency:o.currency});}catch(e){return json(res,503,{success:false,error:e.message});}}
  if(p==='/api/payment/razorpay/verify'&&req.method==='POST'){const x=await bodyJson(req);const j=db.jobs.find(q=>q.id===x.jobId);if(!j)return json(res,404,{success:false,error:'Job not found'});if(j.razorpay_order_id!==x.razorpay_order_id||!verifyRazorpayPayment(x.razorpay_order_id,x.razorpay_payment_id,x.razorpay_signature))return json(res,400,{success:false,error:'Payment verification failed'});j.payment_status='paid';j.payment_method='razorpay';j.razorpay_payment_id=x.razorpay_payment_id;j.status='waiting';save();return json(res,200,{success:true});}
  if(p==='/api/register/create-order'&&req.method==='POST'){const x=await bodyJson(req),plan=String(x.plan||'').toLowerCase(),amount=planPrice(plan);if(!['starter','pro','premium'].includes(plan))return json(res,400,{success:false,error:'Invalid paid plan'});if(!String(x.name||'').trim()||!String(x.phone||'').trim()||!String(x.email||'').trim())return json(res,400,{success:false,error:'Name, mobile and email are required'});try{const regId=id('reg'),o=await razorpayOrder(amount,regId);db.registrations=db.registrations||{};db.registrations[regId]={id:regId,plan,name:String(x.name).trim(),phone:String(x.phone).trim(),email:String(x.email).trim(),shopName:String(x.shopName||x.name).trim().slice(0,100),orderId:o.id,amount,status:'payment_pending',created_at:new Date().toISOString()};save();return json(res,200,{success:true,registrationId:regId,keyId:RZP_KEY_ID,orderId:o.id,amount:o.amount,currency:o.currency,plan});}catch(e){return json(res,503,{success:false,error:e.message});}}
  if(p==='/api/register/verify-payment'&&req.method==='POST'){const x=await bodyJson(req);db.registrations=db.registrations||{};const r=db.registrations[x.registrationId];if(!r)return json(res,404,{success:false,error:'Registration not found'});if(r.status==='paid')return json(res,409,{success:false,error:'Registration already completed'});if(r.orderId!==x.razorpay_order_id||!verifyRazorpayPayment(x.razorpay_order_id,x.razorpay_payment_id,x.razorpay_signature))return json(res,400,{success:false,error:'Payment verification failed'});const sid=generateShopId(),pw=generatePassword(),now=new Date().toISOString();const s={id:sid,name:r.shopName,address:'',phone:r.phone,email:r.email,passwordHash:sha(pw),paused:false,supply_warning:false,subscription_expired:false,shop_payment_mode:'counter_only',online_allowed:false,color_price:10,bw_price:5,agent_token:id('agent'),agent_id:null,plan:r.plan,plan_status:'active',plan_paid:true,plan_amount:r.amount,plan_payment_id:x.razorpay_payment_id,created_at:now,activated_at:now};db.shops.push(s);r.status='paid';r.paymentId=x.razorpay_payment_id;r.shopId=sid;r.completed_at=now;save();return json(res,200,{success:true,shopId:sid,password:pw,plan:r.plan,shop:publicShop(sid)});}
 ...
 if(p==='/api/admin/settings'&&req.method==='POST'){const s=auth(req);if(!s)return json(res,401,{success:false});const x=await bodyJson(req);for(const k of ['name','address','phone','email','color_price','bw_price','paused','supply_warning'])if(k in x)s[k]=x[k];save();return json(res,200,{success:true,shop:publicShop(s.id)});}

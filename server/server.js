...
async function api(req,res,u){
 ...
 if(p==='/api/admin/login'&&req.method==='POST'){const x=await bodyJson(req);const user=String(x.username||'');const pass=String(x.password||'');const envUser=process.env.ADMIN_USER||'admin';const envPass=process.env.ADMIN_PASSWORD||'Admin@12345';if(user!==envUser||pass!==envPass)return json(res,401,{success:false,error:'Invalid admin credentials'});const t=id('adm');db.adminTokens[t]='admin';save();return json(res,200,{success:true,token:t,user:'admin'});}
 ...
 if(p==='/api/admin/settings'&&(req.method==='POST'||req.method==='PUT')){
   ...
   save(); return json(res,200,{success:true,shop:publicShop(s)});
 }

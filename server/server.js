...
function load(){ try{const d=JSON.parse(fs.readFileSync(DB,'utf8')); d.shops=d.shops||[]; d.jobs=d.jobs||[]; d.tokens=d.tokens||{}; d.settings=d.settings||{}; d.events=d.events||[]; d.adminTokens=d.adminTokens||{}; d.pendingRegistrations=d.pendingRegistrations||{}; return d;}catch(e){return {shops:[],jobs:[],tokens:{},settings:{},events:[],adminTokens:{},pendingRegistrations:{}};} }
let db=load();
...
const PLANS={demo:{name:'Demo',price:0,durationDays:1,printLimit:10,advance:false},starter:{name:'Starter',price:599,durationDays:0,printLimit:null,advance:false},pro:{name:'Pro',price:899,durationDays:0,printLimit:null,advance:true},premium:{name:'Premium',price:999,durationDays:0,printLimit:null,advance:true}};
function normalizeShop(s){ if(!s.plan)s.plan='demo'; if(s.plan==='demo'&&!s.demo_expires_at&&s.created_at)s.demo_expires_at=new Date(Date.parse(s.created_at)+86400000).toISOString(); if(typeof s.demo_prints_used!=='number')s.demo_prints_used=0; if(!s.plan_status)s.plan_status='active'; return s;}

function seed(){if(db.shops.length)return;db.shops=[{id:'DEMO',name:'QR Se Print Demo Shop',address:'',phone:'',email:'',passwordHash:sha('1234'),paused:false,supply_warning:false,subscription_expired:false,shop_payment_mode:'both',online_allowed:false,color_price:5,bw_price:3,agent_token:'agent_demo',agent_id:null,agent_last_heartbeat:null,agent_printers:[],plan:'demo',plan_status:'active',demo_expires_at:new Date(Date.now()+86400000).toISOString(),demo_print_limit:10,demo_prints_used:0,created_at:new Date().toISOString()}]; db.adminTokens=db.adminTokens||{}; save();}
db.adminTokens=db.adminTokens||{}; db.shops.forEach(normalizeShop); seed();
...
async function api(req,res,u){
 ...
 if(p==='/api/admin/shops'&&req.method==='GET'){if(!adminAuth(req))return json(res,401,{success:false,error:'Unauthorized'});return json(res,200,{success:true,shops:db.shops.map(s=>({...publicShop(s),plan:s.plan,plan_status:s.plan_status,demo_expires_at:s.demo_expires_at,demo_print_limit:s.demo_print_limit,demo_prints_used:s.demo_prints_used,agent_online:!!s.agent_last_heartbeat&&Date.now()-Date.parse(s.agent_last_heartbeat)<45000}))});}
 if(p==='/api/admin/shop/update'&&req.method==='POST'){if(!adminAuth(req))return json(res,401,{success:false,error:'Unauthorized'});const x=await bodyJson(req);const s=db.shops.find(q=>String(q.id).toUpperCase()===String(x.shopId||'').toUpperCase());if(!s)return json(res,404,{success:false,error:'Shop not found'});for(const k of ['name','phone','email','plan','plan_status','paused','subscription_expired'])if(k in x)s[k]=x[k];if(x.plan&&PLANS[x.plan]){s.plan=x.plan;s.plan_status='active';if(x.plan==='demo'){s.demo_expires_at=new Date(Date.now()+86400000).toISOString();s.demo_print_limit=10;s.demo_prints_used=0;}}save();return json(res,200,{success:true,shop:publicShop(s.id)});}
 ...
 if(p==='/api/register/order'&&req.method==='POST'){
   ...
   const secret=process.env.PLATFORM_RAZORPAY_KEY_SECRET||process.env.RAZORPAY_KEY_SECRET;
 ...
 if(p==='/api/demo/request'&&req.method==='POST'){
   ...
   const pwd=String(x.password||x.mobile||Math.floor(100000+Math.random()*900000)); const created=new Date(); const s={id:sid,name:String(x.shopName||x.name||'Demo Shop').slice(0,100),address:String(x.address||''),phone:String(x.mobile||x.phone||''),email:String(x.email||''),passwordHash:sha(pwd),paused:false,supply_warning:false,subscription_expired:false,shop_payment_mode:'both',online_allowed:false,color_price:10,bw_price:5,agent_token:id('agent'),agent_id:null,agent_last_heartbeat:null,agent_printers:[],plan:'demo',plan_status:'active',demo_expires_at:new Date(created.getTime()+86400000).toISOString(),demo_print_limit:10,demo_prints_used:0,created_at:created.toISOString()}; db.shops.push(s); save(); return json(res,200,{success:true,approved:true,hours:24,printLimit:10,shopId:sid,password:pwd,loginUrl:'/admin',qrCode:shopQrUrl(sid),shop:publicShop(sid)});
 }
 ...
 if(p==='/api/printer-models'&&req.method==='GET')return json(res,200,{success:true,models:['Default Printer','Microsoft Print to PDF','HP LaserJet Pro MFP M126 plus series','EPSON L3210 Series']});
 ...
 if(p==='/api/shop/notice'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false,error:'Unauthorized'});return json(res,200,{success:true,notice:s.notice||''});}
 if(p==='/api/shop/insights'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false,error:'Unauthorized'});const js=db.jobs.filter(j=>j.shopId===s.id);return json(res,200,{success:true,totalJobs:js.length,completed:js.filter(j=>j.status==='completed').length,failed:js.filter(j=>j.status==='failed').length,revenue:js.filter(j=>j.payment_status==='paid'||j.payment_status==='counter').reduce((a,j)=>a+Number(j.amount||0),0)});}
 if(p==='/api/shop/earnings-breakdown'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false,error:'Unauthorized'});const js=db.jobs.filter(j=>j.shopId===s.id);return json(res,200,{success:true,bw:js.filter(j=>!String(j.colorMode||'').toLowerCase().includes('color')).reduce((a,j)=>a+Number(j.amount||0),0),color:js.filter(j=>String(j.colorMode||'').toLowerCase().includes('color')).reduce((a,j)=>a+Number(j.amount||0),0)});}
 if(p==='/api/advance-features'&&req.method==='GET'){return json(res,200,{success:true,features:['4x6 Photo','Resume','A3','Duplex','Mini Print']});}
 if(p==='/api/shop/advance-active'&&req.method==='GET'){const s=auth(req);if(!s)return json(res,401,{success:false});return json(res,200,{success:true,active:['pro','premium'].includes(s.plan)});}
